            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="Supplier_Category_id" id="suppliers_categories_id" class="form-control ">
                            <option value="{{ old('Supplier_Category_id ') }}">اختر الفئة</option>
                            @if (@isset($data) && !@empty($data))
                            @foreach ($data as $info )
                            <option @if(old('Supplier_Category_id',$info['Supplier_Category_id'])==$info->id) selected="selected" @endif
                                value="{{ $info->id }}"> {{ $info->name }} </option>
                            @endforeach
                            @endif
                        </select>
                        @error('Supplier_Category_id') <span class="text-danger">{{ $message }}</span>
                                @enderror
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="active" id="active" class="form-control">
                            ...
                            <option @if(old('active')==1 || old('active')=="" ) selected="selected" @endif value="1">
                                نعم</option>
                            <option @if( (old('active')==0 and old('active')!="" )) selected="selected" @endif
                                value="0"> لا</option>
                        </select>
                        @error('active')
                        <span class="text-danger">{{ $message }}</span>
