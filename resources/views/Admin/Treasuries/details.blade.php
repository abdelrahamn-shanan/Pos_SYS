      ...
      <div class="card">
        ...
        <table id="example2" class="table table-bordered table-hover">
            ...
            <tr>
                ...
                <td > {{ $data['last_recieve_exchange'] }}</td>
            </tr>

            <tr>
                <td class="width30">اخر ايصال تحصيل </td> 
                <td > {{ $data['last_recieve_collect'] }}</td>
            </tr>
            <tr>
                <td class="width30">نوع الخزنة</td> 
                <td > @if($data['is_master']==1) رئيسية  @else غير رئيسية @endif</td>
            </tr>
...
<a href="{{ route('Treasureies_edit',$data['id']) }}" class="btn btn-sm btn-success">تعديل</a>
<a href="{{ route('Treasureies_index') }}" class="btn btn-sm btn-info">عودة</a>
     ...
     <div class="card-header">
        <h3 class="card-title card_title_center">الخزن الفرعية التي سوف تسلم عهدتها الي الخزنة ( {{ $data['name'] }} )  
            ...
            <a href="{{route('add_sub_treasure' , $data->id)}}" class="btn btn-sm btn-primary">اضافة جديد</a>
     ...
     <div id="ajax_responce_serarchDiv">
       ...
       <td>
  <a href="{{route('Treasure_delete',$info->id)}}" class="btn btn-sm btn-danger are_you_shue">حذف</a>
       </td>
